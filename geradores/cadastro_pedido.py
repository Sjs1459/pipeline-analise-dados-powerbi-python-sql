...
def criar_pedidos(conexao):
  cursor = conexao.cursor()
  query_criar_pedido = 'SELECT max(id_pedido) from pedidos' # Seleciona o maior id dos pedidos para continuar dele
  cursor.execute(query_criar_pedido)
  ultimo_pedido = cursor.fetchone()[0]
  ultimo_pedido = 0 if ultimo_pedido is None else ultimo_pedido
  novo_pedido = ultimo_pedido + 1 

  q_selecionar_cliente = 'SELECT id_cliente FROM clientes' # Seleciona todos os Ids dos Clientes para gerar aleatóriamente os pedidos
  cursor.execute(q_selecionar_cliente)
  ids_clientes = cursor.fetchall()
  id_cliente = choice(ids_clientes)[0]

  q_selecionar_idvendedor = 'SELECT id_vendedor from vendedores'# Seleciona todos os Ids dos Vendedores para gerar aleatóriamente os pedidos
