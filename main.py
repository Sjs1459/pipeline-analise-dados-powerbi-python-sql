...
try:
    escolha_ger_inj = int(input()) 
    if escolha_ger_inj == 1: 
      ...
      while True: # Gera a quantidade Vendedores com base na escolha do usuário
        if escolha == 1:
          ...
          limpar_terminal()
          break          
      while True: # Gera a quantidade de Clientes com base na escolha do usuário
        if escolha == 1:
      ...
      while True: # Gera os valores de Categoria e Produtos com base na escolha do usuário
        if escolha == 1:
          ...
          limpar_terminal()
          break        
      while True: # Gera os pedidos de Venda e os Itens dos pedidos
        if escolha == 1:
          ...
          for i in range(1, 43):
            ...
            limpar_terminal()
        break
    elif escolha_ger_inj == 2:
      limpar_terminal()
      while True: # Escolha do modo Manual ou Automatino no Injetor de Dados
        print("Injetar novos Dados - Supabase:\n1 - Manualmente\n2 - Automaticamente")
        try:
          escolha = int(input("R: "))
          if escolha == 1 or escolha == 2:
            limpar_terminal()
      ...
      while True: # Gera a quantidade de Clientes informada pelo usuário
              if escolha == 1:
                ...
                clien.cadastrar_clientes(randint(1,10), conexao)
                limpar_terminal()
                break             
      while True: # Gera os pedidos de Venda e os Itens desses
        if escolha == 1:
          print("Quantos pedidos deseja informar?")
          try:
            qt_pedidos = int(input())
